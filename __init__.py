...
__author__ = 'pythona620'

class multipleentitiy(MycroftSkill):

    def __init__(self):
        super(multipleentitiy, self).__init__(name="multipleentitiy")
    ...
    @intent_handler(IntentBuilder("WereAreYouGoing").require("Did").require("You").require("Remember").build())
    def WereAreYouGoing(self, message): # user wants to know what we've got
        try: # try to open our remember list readonly and give user all phrases
            remlist = open(self.remfile,"r") # open file readonly
            goingPhrase = remlist.read() # read file
            alist = goingPhrase.split("\n") # split line by line and make a nice list
            if not goingPhrase:
               ...
               if len(alist) > 2: # make it nice and shiny with AND and commas
                   search = goingPhrase.rfind("\n")
                   goingPhrase = goingPhrase[:search] + "" + goingPhrase[search+1:]
                   search = goingPhrase.rfind("\n")
                   goingPhrase = goingPhrase[:search] + " and " + goingPhrase[search+1:]
                   goingPhrase = goingPhrase.replace('\n', ', ')
               self.speak_dialog('igoing', {'going': goingPhrase}) # give the user our nice list
    ...
    def shutdown(self):
        super(multipleentitiy, self).shutdown()

    def stop(self):
        pass

def create_skill():
    return multipleentitiy()
